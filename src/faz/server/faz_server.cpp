...
Status FazServiceImpl::hook(ServerContext* context,
...
Status FazServiceImpl::unhook(ServerContext* context,
  ...
  if (hooked_functions_.find(eventType) == hooked_functions_.end()) {
    status = Status(StatusCode::NOT_FOUND, "The given event type does not exist");
  } else {
...
Status FazServiceImpl::event(ServerContext* context,
  ...
  int eventType = request->event_type();
  Any& requestPayload = request->payload();
  Any& responsePayload = response->payload();
  Status status; 
  if (hooked_functions_.find(eventType) == hooked_functions_.end()) {
    status = Status(StatusCode::NOT_FOUND, "The given event type does not exist");
  } else {
    ...
    // the Any objects.
    status = caw_functions_[functionName](requestPayload, responsePayload, kvstore_);
  }
  return status; 
}